...
const allKeywords = [...batch1Keywords, ...batch2Keywords];

let mdTable = `### 📊 Master Keyword Research & Metrics Table (Combined List 1 & List 2)\n\n`;
mdTable += `| # | Focus Keyword | Search Volume | SEO Difficulty (SD) | Paid Difficulty (PD) | CPC (USD) | Category |\n`;
mdTable += `|---|---|:---:|:---:|:---:|:---:|:---:|\n`;

allKeywords.forEach((item, idx) => {
    const category = idx < 31 ? "List 1 (Blog & Informational)" : "List 2 (Dubai Service Commercial)";
    mdTable += `| ${idx + 1} | **${item.kw}** | ${item.sv} | ${item.sd} | ${item.pd} | ${item.cpc} | ${category} |\n`;
});

console.log(`Generated ${allKeywords.length} combined keywords.`);
fs.writeFileSync('c:\\Users\\ssght\\OneDrive\\Desktop\\nufka\\combined_keyword_master_table.md', mdTable, 'utf-8');
