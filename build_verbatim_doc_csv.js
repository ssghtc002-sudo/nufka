...
// Filtered Locations List (Dubai, Gold Souk, Abu Dhabi, Sharjah)
const locations = [
    {
...
function buildCSV() {
    ...
    locations.forEach(loc => {
        const row = [
            ...
            escapeCSV(loc.slug),
            escapeCSV(`Corporate Tax Consultants in ${loc.city_name} | NUFCA`),
            escapeCSV(`Corporate tax consultants in ${loc.city_name} helping businesses register, file and stay compliant under Federal Decree-Law No. 47 of 2022.`),
            escapeCSV(generateVerbatimDocHTML(loc))
        ];
        csvContent += row.join(',') + '\n';
    });
    ...
    console.log(`✅ Fixed Master CSV created without raw schema text at: ${targetPath}`);
}
